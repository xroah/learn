...
function _copy(source, target, options, callback) {
    ...
    const _copyFile = (source, target, cb) => {
        fs.lstat(source, (err, stat) => {
            ...
            if (stat.isDirectory()) {
                ...
                return copyDir(source, target, cb);
            }

            copyFile(source, target, overwrite, err => {
                if (err) {
                    return cb(err);
                }
    ...
    const copyFiles = (source, target, files, cb) => {
    ...
    const copyDir = (source, target, cb) => {
        ...
        readdirAndCheck(source, target, (err, files) => {
            ...
            copyFiles(source, target, files, cb);
        });
    }
